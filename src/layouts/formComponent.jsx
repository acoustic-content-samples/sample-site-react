...
export class FormComponent extends React.Component {
	...
	validateForm() {
		if (!this.state.nameValue && !this.state.emailValue) {
			...
			this.setState({dialogMessageBody:'Name and email are required. Please enter them in the form'});
			return false;
		} else if (!this.state.nameValue) {
			this.setState({dialogMessageTitle:'Error'});
			this.setState({dialogMessageBody:'Name is required. Please enter it in the form'});
			return false;
		} else if (!this.state.emailValue) {
			this.setState({dialogMessageTitle:'Error'});
			this.setState({dialogMessageBody:'Email is required. Please enter it in the form'});
			return false;
		} else {
    ...
    render() {
        ...
        return (
			<div id={contentId} data-renderingcontext-id={contentId} className="form-component grid-x">

              <div id={itemId} className="reveal" data-reveal={""} aria-labelledby="modalTitle" aria-hidden="true" role="dialog">
			  ...
			  { headline ? (<h2 className="headline cell" data-wch-inline-edit="elements.headline.value">{ headline }</h2>) : ('') }
			  <form className="section small-12 columns grid-x">
				<div className="input-group name-group small-12 medium-4 large-4 columns">
				  { name ? (<div className="form-label" data-wch-inline-edit="elements.name.value">{ name }</div>) : ('') }
				  <input type="text" onKeyUp={this.inputNameValue} className="form-input" id="name-input" name="name"/>
				</div>

				<div className="input-group email-group small-12 medium-4 large-4 columns">
				  { email ? (<div className="form-label" data-wch-inline-edit="elements.email.value">{ email }</div>) : ('') }
				  <input type="email" onKeyUp={this.inputEmailValue} className="form-input" id="email-input" name="email"/>
				</div>

				{ enter ? (<button onClick={this.validateForm} data-wch-inline-edit="elements.enter" type="button" data-open={itemId} className="button small-12 medium-4 large-4" id="entry-form">
