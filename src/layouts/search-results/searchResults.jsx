...
import {loadContent, 
        ...
        getContent, 
        subscribe, 
        getRouteForContentId} from 'wch-flux-sdk';
...
export class SearchResults extends React.Component {
    ...
    processResults(res) {
        ...
        this.state.searchResults = this.state.searchResults.concat(res.documents); 
        this._scrollHandler();
    }
    ...
    moreToLoad() {
        return (this.state.numFound > (this.state.rowsPerRequest * this.state.start));
    }
    ...
    render() {
        ...
        if ((this.state.searchResults && this.state.searchResults.length > 0) && this.state.inFlight === 'false' && this.state.numFound === 1) {
        ...
        if ((this.state.searchResults && this.state.searchResults.length > 0) && this.state.inFlight === 'false' && this.state.numFound > 1) {
            itemList = this.state.searchResults.map(item => (
                <div key={item.contentId} className="large-4 medium-6 cell search-result-item">
                    <WchContent contentId={item.contentId} summary={true}/>
                </div>    
            ));
        }
        ...
        const itemsWereNotFound = () => {
            return (this.state.numFound === 0 || this.state.searchError === 'true');
        }

        const listComplete = () => {
            return (itemList.length > 0 && this.state.inFlight === 'false' && (itemList.length === this.state.numFound));
        }
        ...
        const checkLoading = () => {
            return (loadMore() && this.state.searchTerm !== '' && this.state.inFlight === 'false' && this.state.searchError === 'false' );
        } 

        if (this.state.contentData.elements) {
            if (this.state.contentData.elements.suggestedArticles){
                suggestedArticlesId = this.state.contentData.elements.suggestedArticles.value ? this.state.contentData.elements.suggestedArticles.value.id : '';
            }
                ...
                <div>
                    ...
                    <div className="grid-x grid-margin-x">
                        ...
                        <div className="large-10 medium-9 small-8 cell results">
                            ...
                            <div>{this.state.numFound} search result for <strong>"{this.state.searchTerm}"</strong></div>
                            )}
                            {this.state.numFound > 1 && (
                            <div>{this.state.numFound} search results for <strong>"{this.state.searchTerm}"</strong></div>
                            )}
                        </div>
                        )}
                        {this.state.inFlight === 'true' && (
                    ...
                    <div className="grid-x grid-margin-x">
                        <div className="medium-12 cell">
                            ...
                            <div className="results">
                                <p>Searching for "{this.state.searchTerm}" doesn't return any good matches.</p>
                                <p>Try your search again with a different term or browse our recommendations.</p>
                                {suggestedArticlesId.length > 0 && (
                                <div className="medium-12 cell">
                                    <WchContent contentId={suggestedArticlesId} summary={true} />
                                </div>)}  
                            </div>    
                            )}                          
                        </div>  
