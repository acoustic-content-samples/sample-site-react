...
export class SignUp extends React.Component{
    ...
    render(){
        ...
        if (this.props.renderingContext) {
			...
			if (this.props.renderingContext.elements.backgroundImage.renditions) {
				imageURL = getImageUrl(this.props.renderingContext.elements.backgroundImage, 'short', status);
			}
			dialogMessage = this.props.renderingContext.elements.dialogMessage.value;
			backgroundStyle = {
			...
			return (
                <div id={contentId} data-renderingcontext-id={contentId}>
                    <div className="reveal" id={itemId} data-reveal={""} aria-labelledby="modalTitle" aria-hidden="true"
                         role="dialog">
                        <div data-wch-inline-edit="elements.dialogMessage.value" dangerouslySetInnerHTML={createDialogMessage()}></div>
                        <a className="close-button" data-close={""} aria-label="Close">&#215;</a>
                    </div>

                    <section className="grid-container">
                        <div data-wch-inline-edit="elements.backgroundImage" className="newsletter" style={backgroundStyle}>
                            <div className="call-to-action">
                                <h4 data-wch-inline-edit="elements.text.value">{headline}</h4>
                            </div>
                            <div className="wrap-inputs">
                                <form name="emailForm" onSubmit={formPreventSubmit}>
                                    <input onKeyUp={this.inputValue} type="text" className="text-content"
                                           placeholder="Email address" name="emailAddr">
                                    </input>
                                    <button data-wch-inline-edit="elements.link" onClick={this.openModal} type="button"
                                            className="button news-btn float-right"
                                            id="newsletter-subscribe"> {callToAction} </button>
                                </form>
                            </div>
                        </div>
                    </section>
