...
export class ArticleBodyImage extends React.Component {
    ...
    render() {
        ...
        if(this.props.renderingContext) {
			...
			imageUrl = getImageUrl(this.props.renderingContext.elements.image, imageSize);
			...
			return (
                <div id={this.props.renderingContext.id} data-renderingcontext-id={this.props.renderingContext.id} className={className.join(' ')}>
                    <img data-wch-inline-edit="elements.image" src={imageUrl} alt={altText} title={altText}/>
                    <div className="caption" data-wch-inline-edit="elements.imageCaption.value">{caption}
                        <span data-wch-inline-edit="elements.imageCredit.value">{credit}</span>
                    </div>
                </div>
			)
		} else {
