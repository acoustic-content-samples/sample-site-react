...
import 'styles/pages/allTypesPageHero4Blocks.scss';
import {
	getImageUrl,
	getVideoUrl,
	getFileUrl,
...
export class AllTypesPageHero4Blocks extends React.Component {
	render () {
		...
		if (this.props.renderingContext) {
				...
				this.props.renderingContext.elements[cell].values.map(
					(s, index) => {
						...
						return (
							<div
								data-wch-inline-edit={editAccessor}
								className="section large-6 columns safe-padding"
								key={s.id}>
			...
			if (
				...
				sectionOne = (
					<div className="cell medium-6 large-6 columns">
						<div className="grid-x">{sectionsOf('sectionOne')}</div>
					</div>
				);
			}
			...
			if (
				...
				sectionTwo = (
					<div className="cell medium-6 large-6 columns">
						<div className="grid-x">{sectionsOf('sectionTwo')}</div>
					</div>
				);
			}
		}
